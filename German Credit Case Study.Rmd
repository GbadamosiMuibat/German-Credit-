...
# spliting the data
set.seed(234)
train_indices <- sample(seq_len(nrow(credit_data)), size = 0.9 * nrow(credit_data))
...
```{r}
logistic_model <- glm(Risk ~  Saving.accounts + Checking.account + Credit.amount + Duration + Purpose, 
                      data = train_data, 
                      family = binomial,
...
```{r}
table(train_data$Risk, train_data$Sex)
table(train_data$Risk, train_data$Job)
table(train_data$Risk, train_data$Housing)

table(train_data$Risk, train_data$`Saving accounts`)
table(train_data$Risk, train_data$`Checking account`)
